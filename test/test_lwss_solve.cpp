...
SearchPattern create_lwss_search_pattern() {
    ...
    // Interior cells use the LWSS cell group
    pattern.set_cell_group_if(lwss_group, [&](const Cell& c){ return !pattern.is_boundary(c.position); });

    // Boundary cells are known dead with no cell group
    pattern.set_cell_group_if(-1, [&](const Cell& c){ return pattern.is_boundary(c.position); });
...
int main() {
    ...
    for (int t = 0; t < var_grid.size_t(); t++) {
        std::vector<int> at_least_one_alive;
